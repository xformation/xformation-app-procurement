...
class EmailsPage extends Component {
  ...
  render() {
    ...
    return (
      <div>
        {rows && rows.id && <li key={rows.id} >
          <div className="user-id">
            <div className="check-box">
              <FormControlLabel
                control={
                  <Checkbox name="checkedB" color="primary" checked={rows.isChecked} onChange={(e) => this.hamndleSelect(e, otherProps.i)} />
                }
              />
            </div>
            <div className={`user-box ${rows.isStar == "true" ? "active" : ""}`}><StarIcon /></div>
          ...
          <div className="user-content">
            <div className="d-flex">
              <div className="col-9" onClick={this.handleUrl}
                ...
                <div className="d-flex">
                  <div className="user-img">  {rows && rows.sender && rows.sender.profilePic &&
                    <img src={rows.sender.profilePic} alt="" height="50px" width="50px" />}</div>
                  <div className="user-inner-content">
                    <span>{rows.sender.email} {rows.time}</span>
                    <h5>{rows.subject}</h5>
                    <p>{rows.body}</p>
                  </div>
                </div>
              ...
              <div className="col-3 pr-0">
                {!rows.showIcon && <div className="list-icon">
                  <IconButton onClick={() => this.showIcon(otherProps.i)} className="menu-icon">
                    <MoreVertIcon /></IconButton></div>}
                {rows.showIcon && <ButtonGroup variant="text" aria-label="text primary button group">
                  {rows.isRead == "true" && <IconButton>
                    <DirectionsRailwayIcon /></IconButton>}
                  {rows.isSnooze == "true" && <IconButton>
                    <WatchLaterIcon /></IconButton>}
